...
export var dataWeather_Current
export var dataWeather_Forecast

export const getForeCastWeathe_byCityName = (cityName) => {
    fetch(`http://api.openweathermap.org/data/2.5/forecast?q=${cityName}&appid=${API_key}&units=metric&lang=vi`)
        .then(res => res.json())
        .then(data => {
            dataWeather_Forecast = data
            //console.log("Dữ liệu (Dự báo) từ thành phồ: ", cityName, data)
            displayWeatherForeCast(data, data.name)
            displayChartLine(data, data.name)
            setupCalendar()
        })
}
export const getForeCastWeathe_byLatLon= (lat, lon) => {
    fetch(`http://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${API_key}&units=metric&lang=vi`)
        .then(res => res.json())
        .then(data => {
            dataWeather_Forecast = data
            //console.log("Dữ liệu (Dự báo) từ thành phồ: ", cityName, data)
...
export const getCurrenttWeather_byCityName = (cityName) => {
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${API_key}&units=metric&lang=vi`)
        .then(res => res.json())
        .then(data => {
            dataWeather_Current = data
          //  console.log("Dữ liệu (Hiện tại) từ thành phồ: ", cityName, data)
            displayWeather(data, data.name)            
        })
}
export const getCurrenttWeather_byLatLon = (lat, lon) => {
